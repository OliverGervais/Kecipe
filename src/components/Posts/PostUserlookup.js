...
const PostUserlookup = ({setCurrentId}) => {
    ...
    return(
        ...
        !posts.length ? <div>No recipe</div> : (
            ...
            text="text"
            placeholder="User look up ex:Email@gmail.com"
            onChange={(event)=>{
            ...
            />
                ...
                {posts.filter((post) =>{
                    if (search === ""){
                        return post
                    }else if (post.email.toLowerCase().includes(search.toLowerCase())){
                        return post
                    }
                }).map((post )=>{
                    return(
